...
**Phase 1〜5 完了済**（2026-06-18 〜 2026-06-22）。実機で評価・履歴・テーマ・ヘルプ・
CAS（Greek glyph 32×32 含む）が動作。

**現在の優先課題は Phase 6（自前ランチャ + プリベイク bitmap フォント）**。
詳細は [`design/HANDOFF_phase6_launcher_fonts.md`](design/HANDOFF_phase6_launcher_fonts.md) を参照。

2026-06-22 セッションで LofiFren ランチャ経由の起動・終了を繰り返すと深刻なメモリ
断片化に陥り、chrome.bin (51 KB) の確保が失敗してフリーズに見える状態を再現できた。
対策として：

1. 自前ランチャ + `machine.soft_reset()` で起動毎にヒープを完全クリア
2. プリベイク 8×8 bitmap フォント を実装して `_draw_text_*` の framebuf 一時確保を廃止

を Phase 6 として進める。

Phase 5 の plot 実装は 2026-06-22 セッションで一旦完成したが、上記断片化問題で
本体動作確認が滞ったため `git stash@{0}` に退避中。Phase 6 完了後に復元する。
